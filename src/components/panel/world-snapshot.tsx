...
export function WorldSnapshot({ world, defaultOpen = true }: WorldSnapshotProps) {
  return (
    <details className="group rounded-xl border border-slate-200 bg-white shadow-sm p-3 text-sm" open={defaultOpen}>
      <summary className="flex items-center gap-2 cursor-pointer font-medium text-slate-700 hover:text-slate-900 transition-colors list-none [&::-webkit-details-marker]:hidden">
...
function SnapshotRow({ label, value, mono }: { label: string; value: string; mono: boolean }) {
  return (
    <div className="flex justify-between gap-4">
      ...
      <span className={`text-slate-700 text-right truncate ${mono ? 'font-mono text-[11px]' : ''}`}>
        {value}
      </span>
    </div>
  )
